...
Function PoeNoEstoque( cProduto, nQuantidade ) 
   ...
   IF Used() 
      ...
      ELSE 
         cTela:= ScreenSave( 0, 0, 21, 79 ) 
         Aviso( "ESTOQUE: Atualizacao de Entrada do produto " + cProduto + "n�o foi poss�vel...", .T. ) 
         GravaLogTmp( "ESTOQUE! " + "Produto nao localizado...." + " USUARIO: " + StrZero( nGCodUser, 3, 0 ) + Chr( 13 ) + Chr( 10 ) ) 
         ScreenRest( cTela ) 
      ENDIF 
   ENDIF 
...
FUNCTION LancEstoque( cProduto, cCodRed, cEntSai, cDoc, nCodigo, nVlrSdo, ; 
   ...
   IF !( cEntSai $ "+-" ) 
      ScreenRest( cTela ) 
      nEntSai:= SWAlerta( " << PROBLEMA NO ESTOQUE >>; N�o esta definida uma operacao para; esta movimentacao com produtos; O que deseja fazer?", { "Dar Saida", "Dar Entrada", "Nao Atualizar Estoque" } ) 
      IF nEntSai==1 
         cEntSai:= "+" 
      ELSE 
         cEntSai:= "-" 
