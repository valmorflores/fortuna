...
//      EndDo 
//   Else 
//      Aviso( "Nenhuma cotaáÑo foi selecionada...", 24 /2 ) 
//      Pausa() 
//      SetColor( cCor ) 
//      SetCursor( nCursor ) 
//      ScreenRest( cTela ) 
   ...
   whil .t. 
      ...
      do case 
              ...
              IF Empty( CODPED ) 
                 ...
                 For nCt:= 1 To Len( aCotacoes ) 
                     If DBSeek( aCotacoes[ nCt ][ 1 ] ) 
                        ...
                        DBUnLock() 
                     Else 
                        Aviso( " Cotacao nß " + aCotacoes[ nCt ][ 1 ] + " nao foi encontrada...", 24 / 2 ) 
                        Pausa() 
                     EndIf 
                 Next 
                 ...
                 If DBSeek( cCodCot ) 
                    ...
                    EndDo 
                 EndIf 
                 Aviso( "Foi gerado o pedido de Nß " + cCodPed + "...", 24 / 2 ) 
         ...
         case nTecla==K_F2 
              ...
              @ 12,12 Say "DescriáÑo..: [" + LEFT( aPedido[ nRow ][ 2 ], DESCRI_TAMANHO ) + "]" 
              @ 13,12 Say "Preáo......:" Get nPrecoInicial Pict "@E 999,999,999.999" 
              @ 14,12 Say "% Desconto.:" Get nPerDesconto  Pict "@E 999.99" Valid {|oGet| CalculaDesconto( oGet, GetList, 3, 5 ) } 
              @ 15,12 Say "Preáo Final:" Get nPrecoFinal   Pict "@E 999,999,999.999" 
...
whil .t. 
   ...
   do case 
      ...
      case Chr( nTecla ) $ "Gg" .OR. nTecla == K_TAB 
           ...
           @ 12,11 Say " ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ∞€∞€∞€" 
           ...
           ELSE 
              nTroco:= 0 
           ENDIF 
           @ 17,11 Say " Trocoƒƒƒƒƒƒƒƒƒƒƒƒ-" + Tran( nTroco, "@E 999,999,999,999.99" ) 
      ...
      case nTecla==K_ENTER 
           ...
           @ 11,12 Say "DescriáÑo..: [" + LEFT( aPedido[ nRow ][ 2 ], DESCRI_TAMANHO ) + "]" 
           @ 12,12 Say "Fabricante.: [" + cOrigem + "]" 
           @ 13,12 Say "Preáo......: [" + Tran( nPrecoInicial, "@E 999,999,999.999" ) + "]" 
           VPBox( 18, 10, 20, 70,, _COR_GET_BOX, .T., .F., _COR_GET_TITULO ) 
           @ 19,12 Say "Quantidade.:" Get nQuantidade   Pict "@E 999,999.999" 
           DisplayIPI( nPrecoFinal ) 
           Read 
...
/* 
* Modulo      - CalculoGeral 
* Finalidade  - Apresentar no rodapÇ o calculo total do pedido 
* Programador - Valmor Pereira Flores 
* Data        - 26/Outubro/1995 
* Atualizacao - 
*/ 
...
Static Function VerCodigo( cCodigo, GetList ) 
   ...
   If !DBSeek( cGrupo_ + cCodigo + Space( 5 ) ) 
      ...
      Aviso( "C¢digo nÑo existente neste grupo...", 24 / 2 ) 
...
While .T. 
   ...
   do Case 
      Case nTecla==K_INS 
           ...
           @ 10,40 Say "I.E.:" Get cInscri When Mensagem( "Digite o nß da inscricao estadual." ) 
           @ 11,18 Get dDataCd 
           READ 
           IF LastKey() == K_ESC 
              SetCursor( 0 ) 
   ...
   While .T. 
      ...
      IF mpr->Marca_ == "*" 
         IF AScan( aPedido, {|x| x[11] == Val( MPr->Indice ) .AND. ! ( x[11] == 0 ) } ) > 0 
            SetColor( "15/00" ) 
            @ 12,36 SAY "ªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªªª" 
         ELSE 
            SetColor( "15/04" ) 
      ...
      do case 
         ...
         case nTECLA=K_ENTER 
              ...
              @ 11,12 Say "DescriáÑo..: [" + Alltrim( MPR->Descri ) + "]" 
              @ 12,12 Say "Fabricante.: [" + cOrigem + "]" 
              @ 13,12 Say "Preáo......: [" + Tran( nPrecoInicial, "@E 999,999,999.999" ) + "]" 
              VPBox( 18, 10, 20, 70,, _COR_GET_BOX, .T., .F., _COR_GET_TITULO ) 
              @ 19,12 Say "Quantidade.:" Get nQuantidade   Pict "@E 999,999.999" 
         ...
         case nTECLA=K_INS 
              ...
              @ 11,12 Say "DescriáÑo..:" Get cDescricao Pict "@!" 
...
IF !Empty( aPrBloqueados ) 
   Mensagem( ">>>" + STRZero( Len( aPrBloquados ), 2, 0 ) + " do(s) " + STRZero( Len( aPedidos ), 2, 0 ) + " produto(s) nÑo foram desmarcado(s)..." ) 
   Pausa() 
ENDIF 
...
*/ 
  ...
  IF !DBSeek( nCodigo ) 
    ...
    */ 
    aNumero:={ "    ‹‹ ‹‹‹‹‹ ‹‹‹‹‹ ‹   ‹ ‹‹‹‹‹ ‹‹‹‹‹ ‹‹‹‹‹ ‹‹‹‹‹ ‹‹‹‹‹ ‹‹‹‹‹",; 
               "     €     €     € €   € €     €         € €   € €   € €   €",; 
               "     € €ﬂﬂﬂﬂ  ﬂﬂﬂ€ ﬂﬂﬂﬂ€ ﬂﬂﬂﬂ€ €ﬂﬂﬂ€     € €ﬂﬂﬂ€ ﬂﬂﬂﬂ€ €   €",; 
               "     € €‹‹‹‹ ‹‹‹‹€     € ‹‹‹‹€ €‹‹‹€     € €‹‹‹€ ‹‹‹‹€ €‹‹‹€" } 
