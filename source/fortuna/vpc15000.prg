...
vpobsbox(.T.,10,23," Sobre: Reindexacao ",; 
               {"                                                ",; 
                ...
                " e anormalidades voc� dever� execut�-lo.        ",; 
                "                                                ",; 
                "                                                "}) 
...
WHILE !EMP->( EOF() ) 
    ...
    ELSE 
       ...
       ENDIF 
       // Se a empresa � 0 n�o deve ter \0000 no diretorio 
       IF nEmpresa==0 
          IF AT( "\0000", GDir ) > 0 
...
// ## CL2HB-ERRO - Talvez estas linhas ainda necessitem de correcoes, verifique
    ...
    FOR nFiles:= 1 TO 255 
        IF nFiles <> 70 
           ...
           // Filtra arquivos de empresas que s�o genericos 
           // para que n�o execute reindex duas vezes nos arquivos. 
           IF ( nEmpresa == 0 ) .OR.; 
...
//if file(ARQUIVO) 
  ...
  //// VALMOR: Fim do reposicionamento, embora continue reposicionado 
   ...
   // ELAS S�O FUNDAMENTAIS PARA O BOM FUNCIONAMENTO DA REINDEXACAO 
...
nVlr:= INT( nVlr / 3 ) 
@ 10,39 say "�" + Repl( "�", nVlr ) Color "15/" + CorFundoAtual() 
@ ROW(), COL() Say Repl( "�", 35 - nVlr ) Color "14/" + CorFundoAtual() 
setcolor(cCOR) 
return(.t.) 
