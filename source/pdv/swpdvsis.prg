...
#Define SUPORTE_MODO_TEXTO

#Define _PDVCOD_MPRIMA        10
#Define _PDVVPB_MPRIMA        &DiretorioDeDados\CDMPRIMA.DBF

#Define _PDVCOD_PRECOAUX      11
#Define _PDVVPB_PRECOAUX      &DiretorioDeDados\TABAUX__.DBF

#Define _PDVCOD_ESTOQUE       12
#Define _PDVVPB_ESTOQUE       &DiretorioDeDados\CESTOQUE.DBF

#Define _PDVCOD_CLIENTE       13
#Define _PDVVPB_CLIENTE       &DiretorioDeDados\CLIENTES.DBF

#Define _PDVCOD_BANCO         14
#Define _PDVVPB_BANCO         &DiretorioDeDados\CDBANCOS.DBF

#Define _PDVCOD_VENDEDOR      15
#Define _PDVVPB_VENDEDOR      &DiretorioDeDados\VENDEDOR.DBF

#Define _PDVCOD_CAIXA         16
#Define _PDVVPB_CAIXA         &DiretorioDeDados\CAIXA___.DBF

#Define _PDVCOD_DUPAUX        17
#Define _PDVVPB_DUPAUX        &DiretoriodeDados\CDDUPAUX.DBF

#Define _PDVCOD_CUPOM         18
#Define _PDVVPB_CUPOM         &DiretoriodeDados\CUPOM___.DBF

#Define _PDVCOD_CUPAUX        19
#Define _PDVVPB_CUPAUX        &DiretoriodeDados\CUPOMAUX.DBF
...
#define _PDVCOD_PDVEAN        20
#define _PDVVPB_PDVEAN        &DiretoriodeDados\PDVEAN__.DBF

#define _PDVCOD_CORES         21
#define _PDVVPB_CORES         &DiretoriodeDados\CORES___.DBF

#define _PDVCOD_CLASSES       22
#define _PDVVPB_CLASSES       &DiretoriodeDados\CLASSES_.DBF

#Define _PDVCOD_ASSEMBLER     23
#define _PDVVPB_ASSEMBLER     &DiretoriodeDados\MONTAGEM.DBF

#Define _PDVCOD_CONDICOES     24
#Define _PDVVPB_CONDICOES     &DiretorioDeDados\CONDICAO.DBF

#Define _PDVCOD_TABPRECO      25
#Define _PDVVPB_TABPRECO      &DiretorioDeDados\TABPRECO.DBF

#Define _PDVCOD_TABAUX        26
#Define _PDVVPB_TABAUX        &DiretorioDeDados\TABAUX__.DBF

#Define _PDVCOD_GRUPO         27
#Define _PDVVPB_GRUPO         &DiretorioDeDados\GRUPOPRO.DBF
...
#ENDIF
   ...
   SayStr(    2,  3, 4, 0, 15, "  [      ] Aux  [   ] Quant  [   ] Canc  [        ] Venda  [   ] Cliente  [   ] Cond  [   ] Fecha")

   SayStr(   44,  4, 4, 0,  0, "Home" )
   SayStr(  232,  4, 4, 0,  0, "F1" )
   SayStr(  406,  4, 4, 0,  0, "F4" )
   SayStr(  577,  4, 4, 0,  0, "ENTER" )
   SayStr(  816,  4, 4, 0,  0, "F8" )
   SayStr( 1012,  4, 4, 0,  0, "F7" )
   SayStr( 1175,  4, 4, 0,  0, "F6" )

   SayStr(   42,  3, 4, 0, 14, "Home" )
   SayStr(  230,  3, 4, 0, 14, "F1" )
   SayStr(  404,  3, 4, 0, 14, "F4" )
   SayStr(  575,  3, 4, 0, 14, "ENTER" )
   SayStr(  814,  3, 4, 0, 14, "F8" )
   SayStr( 1010,  3, 4, 0, 14, "F7" )
   SayStr( 1173,  3, 4, 0, 14, "F6" )
   ...
   WHILE !LastKey()==K_ESC .AND. lFim == .F.
      ...
      SayStr( 1140, 580, 4, 0, 0, cVersao ) // Versao do Programa

      IF !( Gaveta == Impressora )       /* Impressora diferente da gaveta */
         IF ! ( Driver=="" )
            SayStr( 0830, 0825, 4, 0, 0, ALLTRIM( Impressora ) + " < " + Driver + " > " + ALLTRIM( Gaveta ) + " Em " + DTOC( DataEmissao() ) )
         ELSE
            SayStr( 0830, 0825, 4, 0, 0, ALLTRIM( Impressora ) + " <> " + ALLTRIM( Gaveta ) + " Em " + DTOC( DataEmissao() ) )
         ENDIF
      ELSE
         IF !( Driver=="" )
            SayStr( 0830, 0825, 4, 0, 0, Impressora + " < " + Driver + " > Em " + DTOC( DataEmissao() ) )
         ELSE
            SayStr( 0830, 0825, 4, 0, 0, Impressora + " Em " + DTOC( DataEmissao() ) )
         ENDIF
      ENDIF

      PdvBrowse( @lFim, @lCupomPendente, @lVersaoConflito, @cVersao, nHandInfo, nHandProd, nHandDisp, nHandLogo )
...
ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
*/
Function PdvBrowse( lFim, lCupomPendente, lVersaoConflito, cVersao, nHandInfo, nHandProd, nHandDisp, nHandLogo )
 ...
 // ENDIF
  ...
  IF File( DiretorioDeDados-"\PDVEAN__.DBF" )
      DBSelectAr( _PDVCOD_PDVEAN )
      Use _PDVVPB_PDVEAN Alias EAN Shared
   ...
   IF File( DiretorioDeDados-"\TABPRECO.DBF" )
       DBSelectAr( _PDVCOD_TABPRECO )
       Use _PDVVPB_TABPRECO Alias PRE Shared
   ...
   IF File( DiretorioDeDados-"\TABAUX__.DBF" )
       DBSelectAr( _PDVCOD_TABAUX )
       Use _PDVVPB_TABAUX Alias TAX Shared
   ...
   IF File( DiretorioDeDados-"\CORES___.DBF" )
       DBSelectAr( _PDVCOD_CORES )
       Use _PDVVPB_CORES Alias COR Shared
   ...
   IF File( DiretorioDeDados-"\CLASSES_.DBF" )
       DBSelectAr( _PDVCOD_CLASSES )
       Use _PDVVPB_CLASSES Alias CLA Shared
   ...
   IF File( DiretorioDeDados-"\CLIENTES.DBF" )
       DBSelectAr( _PDVCOD_CLIENTE )
       AbreFile( _PDVCOD_CLIENTE )
   ENDIF

  // Abertura do arquivo de Materia-Prima - Produtos
  IF File( DiretorioDeDados-"\CDMPRIMA.DBF" )
      DBSelectAr( _PDVCOD_MPRIMA )
      Use _PDVVPB_MPRIMA Alias MPR Shared
  ...
  SayStr( 35, 820, 4, 0, 0, "Codigo Fabrica . . . . . . . . . . . . ." )
  SayStr( 35, 780, 4, 0, 0, "Unidade de Medida . . . . . . . . . . " )
  SayStr( 35, 740, 4, 0, 0, "Quantidade  . . . . . . . . . . . . . . ." )
  SayStr( 35, 700, 4, 0, 0, "Origem/Fabricante/ICMs . . . . . . . . " )
  SayStr( 35, 660, 4, 0, 0, "Saldo em Estoque . . . . . . . . . . . "  )
  SayStr( 35, 620, 4, 0, 0, "Status . . . . . . . . . . . . . . . . . . . "  )
  SayStr( 35, 580, 4, 0, 0, "Cliente  . . . . . . . . . . . . . . . . . . "  )

  loadcSet( 0, "VSYS14.PTX" )
  SayStr( 35, 500, 4, 0, 0, "Total" )

  /* Informacoes de Baixo */
  loadcSet( 0, "AGSYSD10.PTX" )
  SayStr( 35, 125, 4, 0, 0, "Ordenacao        " )
  SayStr( 35, 100, 4, 0, 0, "Busca            " )
  SayStr( 35, 075, 4, 0, 0, "Codigo Interno   " )
...
//       cBusca:= ""; lAchou := .T.
      ELSE
         SnapPaste( 220,  75, nHandInfo )
         SayStr( 250, 125, 4, 0, 1, PAD( Alltrim( STR( MPR->( INDEXORD() ) ) ), 20 ) )
         SayStr( 250, 100, 4, 0, 1, PAD( cBusca, 28 ) )
      ENDIF
      ...
      IF !nUltReg==RECNO() .OR. lSubTotal
         ...
         /* Exibicao c/ Sombra */
         IF ( !ALLTRIM( MPR->Detal1 ) == "" ) .OR. ( !ALLTRIM( MPR->Detal2 ) == "" ) .OR.;
            ( !ALLTRIM( MPR->Detal3 ) == "" ) .OR. ( !ALLTRIM( MPR->Detal4 ) == "" )
            loadcSet( 0, "AGSYSD10.PTX" )
            SayStr( 37, 957, 4, 0, 00, MPR->DESCRI )
            SayStr( 34, 942, 4, 0, 11, MPR->DETAL1 )
            SayStr( 34, 925, 4, 0, 11, MPR->DETAL2 )
            SayStr( 34, 908, 4, 0, 11, MPR->DETAL3 )
         ELSE
            loadcSet( 0, "RMN1914.PTX" )
            SayStr( 37, 915, 4, 0, 00, MPR->DESCRI )
            SayStr( 34, 920, 4, 0, 14, MPR->DESCRI )
         ENDIF

         SnapPaste( 400, 500, nHandDisp )
         LoadCSet( 0, "VSYS14.PTX" )

         SayStr( 400, 820, 4, 0, IF( EMPTY( MPR->CODFAB ), 15, 1 ), IIF (EMPTY (MPR -> CODFAB) = .F., MPR -> CODFAB, "~~"))
         SayStr( 400, 780, 4, 0, IF( EMPTY( MPR->UNIDAD ), 15, 1 ), IIF (EMPTY (MPR -> UNIDAD) = .F., MPR -> UNIDAD, "~~" ))
         SayStr( 400, 740, 4, 0, IF( EMPTY( nQuantidade ), 15, 1 ), IIF (EMPTY (nQuantidade) = .F., Alltrim( Tran( nQuantidade, "@E 999,999.999" ) ), "~~"))
         SayStr( 400, 700, 4, 0, IF( EMPTY( MPR->ORIGEM ), 15, 1 ), IIF (EMPTY (MPR->ORIGEM) = .F., MPR->ORIGEM + "  "+IF( !EMPTY( MPR->TABRED ), "[" + MPR->TABRED + "]", ""), "~~"))
         SayStr( 400, 660, 4, 0, IF( EMPTY( MPR->SALDO_ ), 15, 1 ), IIF (EMPTY (MPR->SALDO_) = .F., Alltrim( Str( MPR->SALDO_ ) ), "~~"))

         SnapPaste( 220,  75, nHandInfo )
         SayStr( 250, 125, 4, 0, 1, PAD( Alltrim( STR( MPR->( INDEXORD() ) ) ), 20 ) )
         SayStr( 250, 100, 4, 0, 1, PAD( cBusca, 28 ) )
         SayStr( 250, 075, 4, 0, 1, PAD( ALLTRIM( MPR->INDICE ) + "-" + ALLTRIM( STR( PrecoConvertido() ) ) + "00", 40 ) )

         SayStr( 400, 580, 4, 0, IF( EMPTY( cCliDescri ), 15, 1 ), ;
            IIF (EMPTY (cCliDescri) = .F., ;
            LEFT (Alltrim( cCliDescri ), 18) + " ...", "~~"))

         IF nStatus==10
            SayStr( 400, 620, 4, 0, 2, aStatus[ nStatus ] )
         ELSE
            SayStr( 400, 620, 4, 0, IF( nStatus==4, 4, 1 ), aStatus[ nStatus ] )
         ENDIF
      ...
      IF lSubTotal
         ...
         IF nStatus==10
            SayStr( 400, 500, 4, 0, 08, Alltrim( Tran( PrecoConvertido(), "@E 999,999.99" ) ) )
            SayStr( 400, 502, 4, 0, 03, Alltrim( Tran( PrecoConvertido(), "@E 999,999.99" ) ) )
         ELSE
            SayStr( 400, 500, 4, 0, 7, Alltrim( Tran( nTotal, "@E 999,999.99" ) ) )
            SayStr( 400, 502, 4, 0, 1, Alltrim( Tran( nTotal, "@E 999,999.99" ) ) )
         ENDIF
         //lSubTotal:= .F.
      ENDIF
      //mCurOn()
      ...
      DO CASE
         ...
         CASE nTecla==K_BS .OR. nTecla==K_LEFT .OR. nTecla==K_RIGHT
            IF (EMPTY (cBusca) == .F.)
               ...
               LoadcSet( 0, "VSYS14.PTX" )
               SayStr( 250, 100, 4, 0, 1, PAD( cBusca, 29 ) )
            ENDIF
         ...
         CASE nTecla==K_F8
              ...
              // DADOS DO CLIENTE
              IF nCliCodigo > 0 .And. lGAtualiz
                 SWGAviso( "CADASTRO DE CLIENTES",;
                           {{0, " "},;
                            {0, "   Confirma Atualizacao do Cadastro do Cliente?"},;
                            {0, " "},;
                            {0, "   Pressione [S/N]" },;
                            {0, " "}}, 7, 4)
   ...
   Function DispCupom( cProduto, nCor, lDupla )
      ...
      _VScroll( 778, 498, 1340, 840, 20, 15 )
       ...
       IF !lDupla == Nil .AND. lDupla == .T.
          SayStr( 778, 498, 4, 0, 0, cProduto )
          SayStr( 780, 500, 4, 0, 0, cProduto )
       ELSE
          FOR nCt:=  1 TO LEN( cProduto )
             SayStr( 800, 500, 4, 0, nCor, Substr( cProduto, 1, nCt ) )
          NEXT
          SayStr( 800, 500, 4, 0, nCor, cProduto )
       ENDIF
...
// ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
   ...
   Function DispValor( cValor, nCor )
       ...
       loadcSet( 0, "AGSYSA14.PTX" )
//     loadcSet( 0, "AGSYSD10.PTX" )
       SayStr(1235, 500, 4, 0, nCor, cValor )
...
// ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
   ...
   Function SWGBox( nCol1, nLin1, nCol2, nLin2, cTitulo, nCor, nTipo )
      ...
      IF ModoDeVideo == 0
         // DispBox
         @ nLin1/48, nCol1/14 to nLin2/48, nCol2/14 color StrZero( nCor, 2, 0 ) + "/00"
      ELSE
      #ENDIF
      ...
      */
      //#02-F
         ...
         IF !cTitulo==Nil
            ...
            SayStr( nCol1+15, nLin2+5, 5, 0,  0, cTitulo ) // PRETO
            SayStr( nCol1+12, nLin2+8, 5, 0, 14, cTitulo ) // AMARELO
         ENDIF
      ENDIF
      loadcSet( 0, "VSYS14.PTX" )
...
//                            ÀÄÄÄÄ 2§ elemento = CARACTERES
   ...
   IF !cTitulo==Nil
      SayStr( nCol1+20, nLin2-45, 5, 0,  0, cTitulo )
      SayStr( nCol1+18, nLin2-43, 5, 0, 14, cTitulo )
   ENDIF

   FOR nCt:= Len( aAviso ) TO 1 Step -1
       SayStr( nCol1+30, (nLin2-50) - ( nCt * 50 ), 5, 0, aAviso[ nCt, 1 ], aAviso[ nCt, 2 ] )
       IF (LEFT (aAviso[ nCt, 2 ], 1) == "")
          SayStr( nCol1+32, (nLin2-52) - ( nCt * 50 ), 5, 0,       0, "")
//        SayStr( nCol1+30, (nLin2-50) - ( nCt * 50 ), 5, 0,       4, "")
          SayStr( nCol1+30, (nLin2-50) - ( nCt * 50 ), 5, 0, nCorTit, "")
       ENDIF
   NEXT
...
Local xRetornar:= Nil
   ...
   ELSEIF xPar1 == _ABRIR
      ...
      IF !Used()
         ...
         IF !Used()
            @ ROW(),0 SAY "O sistema PDV ja esta em uso neste computador."
            Inkey(0)
            SetText()
            ffim()
            Quit
...
Local nCt:= 0, lCancelado:= .F.
  ...
  IF !USED()
     AbreBaseCupom()
  ENDIF
  DBSelectAr( _PDVCOD_CUPAUX )
  IF !USED()
     Use _PDVVPB_CUPAUX Alias CAU Shared
...
IF !nLin==Nil
   IF nLin > 0
      ...
      loadcSet( 0, "VSYS14.PTX" )
      SayStr( nCol+14, nLin+4, 4, 0, 8, cMensagem )
      SayStr( nCol+12, nLin+6, 4, 0, 11, cMensagem )
      LoadcSet( 0, "VSYS14.PTX" )
   ENDIF
ENDIF
SetCursor( nCursor )
...
// ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
   ...
   Return (.T.)
      ...
      loadcSet( 0, "VSYS14.PTX" )
      SayStr( 35, 500, 4, 0, 0, cTitulo )

      LoadcSet( 0, "DGE1609.PTX" )
      SayStr( 400, 500, 4, 0, 7, Alltrim( Tran( nValor, "@E 999,999.99" ) ) )
      SayStr( 400, 502, 4, 0, 1, Alltrim( Tran( nValor, "@E 999,999.99" ) ) )
...
Local nSaveFP:= SWSnapCopy( 50, 100, 1260, 900, 0, "PDV_BVEN.$$$" )
   ...
   DBSelectAr( _PDVCOD_VENDEDOR )
   IF !Used()
      Use _PDVVPB_VENDEDOR Alias VEN Shared
...
Local nCt
  DBSelectAr( _PDVCOD_CAIXA )
  Use _PDVVPB_CAIXA Alias CX Shared
...
Local nCt
  ...
  ENDIF
  /// Estoque
  DBSelectAr( _PDVCOD_ESTOQUE )
  Use _PDVVPB_ESTOQUE Alias EST Shared
...
Function LancaReceber( aParcelas, nCliCodigo, cCliDescri )
   ...
   DBSelectAr( _PDVCOD_DUPAUX )
   Use _PDVVPB_DUPAUX Alias DPA Shared
   IF File( DiretoriodeDados + "\DPAIND05.NTX" )
      Set Index To &DiretoriodeDados\DPAIND01.NTX,;
                   &DiretoriodeDados\DPAIND02.NTX,;
                   &DiretoriodeDados\DPAIND03.NTX,;
...
DO CASE
   CASE nCodigoArquivo==_PDVCOD_CLIENTE
       ...
       **********
       ...
       IF !Used()
          RDDSetDefault( "DBFNTX" )
          Use _PDVVPB_CLIENTE Alias CLI Shared
...
Local nArea:= Select(), nPercentual
 DBSelectAr( _PDVCOD_GRUPO )
 DBCloseArea()
 USE _PDVVPB_GRUPO Alias GR0
 IF Used()
...
Function AbreBaseCupom()
      Use _PDVVPB_CUPOM Alias CUP Shared
...
IF !MAISINFO->( DBSeek( CLI->CODIGO ) )
   MAISINFO->( DBAppend() )
   MAISINFO->( RLock() )
   Replace maisinfo->CODIGO With CLI->CODIGO
ENDIF
...
oGTAB:=tbrowsenew(19,25,22,67)
oGTAB:addcolumn(tbcolumnnew(,{|| left( MT[WROW], 40 ) }))
oGTAB:AUTOLITE:=.f.
...
//   @ 01, 00 Say MAISINFO->DESCRI+STR(MAISINFO->CODIGO)
   ...
   End
   nTecla:=inkey(0)
   if nTecla==K_ESC .or. nTecla==K_PGDN  ;exit   ;endif
   do case
      ...
      case nTecla==K_ENTER .OR.;
           ...
           ENDIF
           cInformacao:= MT[WROW]
           @ ROW(),COL() Get cInformacao Pict "@S40"
           READ
           IF LastKey() <> K_ESC
              MT[WROW]:= cInformacao
              //keyboard Chr( LastKey() )
...
nCampo:= 1
while i <= len( MT )
    Repl MAISINFO->&( "INFO"+STRZERO(nCampo,2) ) with MT[i]
    Repl MAISINFO->&( "COMP"+STRZERO(nCampo,2) ) with MT[i+1]
    Repl MAISINFO->&( "SUBC"+STRZERO(nCampo,2) ) with MT[i+2]
    i:= i + 3
    nCampo:= nCampo + 1
enddo
Setcursor(0)
...
Function SayStr( nLeftCol, nBottomLin, x, y, n1,  n2, Text )
  IF ModoDeVideo == 0
     #IFDEF SUPORTE_MODO_TEXTO
     @ ( nLeftCol / 15 ), 24 - ( nBottomLin / 15 ) Say Text COLOR "15/01"
     #ENDIF
  ELSE
     SayString( nLeftCol, nBottomLin, x, y, n1,  n2, Text )
  ENDIF
