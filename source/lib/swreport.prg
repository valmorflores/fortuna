...
//cFile:= UPPER( cFile )
IF AT( "NFISCAL", Upper( cFile ) ) > 0
   IF AT( "0", GDir ) > 0
      nCodEmpresa:= VAL( SubStr( GDir, AT( _PATH_SEPARATOR + "0", GDir ) + 2 ) )
      IF FILE( cDiretorio - _PATH_SEPARATOR - ( cArq:= LEFT( cFile, AT( ".", cFile ) ) + StrZero( nCodEmpresa, 3, 0 ) ) )
         cFile:= cArq
      ENDIF
   ENDIF
ENDIF
...
IF FILE( ALLTRIM( cDiretorio - _PATH_SEPARATOR - cFile ) )
   ...
   ENDIF
ELSE
   Aviso( "Arquivo " + cDiretorio - _PATH_SEPARATOR - cFile +" nao encontrado nesta pasta...", 12 )
   Pausa()
   lRelatorio:= .F.
ENDIF
...
Local aSubChar
IF SET( 20 )=="PRINT" .OR. SET( 20 )=="PRINTER" .OR. nControl<>Nil
...
WHILE .T.
   ...
   IF lLeFormatos
      ...
      aImpressao:= SWFileToArray( cCAMPO )
      ...
      FOR nCt:=1 TO Len( aImpressao )
          ...
          IF LEFT( UPPER( LTRIM( aImpressao[nCt] ) ), 8 ) == "$INCLUDE"
             IF !File( ALLTRIM( SWSet( _SYS_DIRREPORT ) - _PATH_SEPARATOR - ALLTRIM( SUBSTR( aImpressao[nCt], 9 ) ) ) )
                Aviso( "Arquivo " + ALLTRIM( SWSet( _SYS_DIRREPORT ) - _PATH_SEPARATOR - ALLTRIM( SUBSTR( aImpressao[nCt], 9 ) ) ) + " nao foi localizado. ", 12 )
                Pausa()
             ENDIF

             cDefines:= MEMOREAD( ALLTRIM( SWSet( _SYS_DIRREPORT ) - _PATH_SEPARATOR - ALLTRIM( SUBSTR( aImpressao[nCt], 9 ) ) ) )
...
//        IF !lPulaLinha .AND. Alltrim( aImpFormat[ nCt ] ) == Chr( 13 ) + Chr( 10 )
          ...
          ELSE
             ...
             // FIXED: 04/09/2003 VALMOR
                     DEVOUT( RTRIM( aImpFormat[nCt] ) )
//             DEVOUT( aImpFormat[nCt] )           // gelson (comentei esta e descomentei a anterior.)
             ...
             IF lPassadaDupla
                ...
                // FIXED: 04/09/2003 VALMOR
                        DEVOUT( RTRIM( aImpFormat[nCt] ) )
                //DEVOUT( aImpFormat[nCt] ) // valmor Comentei esta linha e descomentei a anterior
             ENDIF
...
FUNCTION Linha( nLin )
IF nLin == NIL
   Return( IF( Set(20)=="PRINT" .OR. SET( 20 )=="PRINTER", Prow(), Row() ) )
ELSE
   nLin+= IF( Set(20)=="PRINT" .OR. SET( 20 ) == "PRINTER", Prow(), Row() )
   @ nLin,00 SAY Space(0)
ENDIF
RETURN NIL
...
FUNCTION ArrobaSay( nLin, nCol, cString )
 ...
 */
 ...
 Function TiraLinhaBranco( cFile, nColunas )
 Local nArea:= Select()
 ...
 WHILE !EOF()
     IF !EMPTY( ORIGEM )
        IF nColunas <> Nil
           @ PROW(),PCOL() Say PAD( SUBSTR( ORIGEM, 1, nColunas ), nColunas ) + Chr( 13 ) + Chr( 10 )
        ELSE
...
Function FileLe( cFile )
 ...
 Local cDiretorio
 Local hArquivo, nTamanho, I, cString
 ...
 cArq:= cFile
 IF File( cArq )
    hArquivo:= FOpen( cArq, FC_NORMAL )
    nTamanho:= Fseek( hArquivo, 0, FS_END )
    Fseek( hArquivo, 0 )
    cInformacao:= FReadStr( hArquivo, nTamanho )
    FClose( hArquivo )
//    @ 00,00 say cInformacao Color "00/02"
//    inkey(0)

    cString:= "" 
    FOR i:= 1 TO Len( cInformacao )
        IF SubStr( cInformacao, i, 2 )== Chr( 13 ) + Chr( 10 )
           DBAppend()
           Replace DESCRI With cString
           cString:= ""
        ELSE
           IF !( SubStr( cInformacao, i, 1 ) == Chr( 10 ) )
              cString:= cString + SubStr( cInformacao, i, 1 )
           ENDIF
        ENDIF
    NEXT
//    @ 01,01 SAY cArq color "00/04"
//    @ 02,01 say cFileCriar Color "00/01" 
//    inkey(0)
    //Append From &cArq SDF
 ENDIF
 Release cArq
...
// inkey(0)
// !command.com
// @ 01,01 say STR( lastrec() ) COLOR "00/14"
// inkey(0)
 WHILE !EOF()
    //@ 01,01 say descri
    //inkey(0)
    AAdd( aArray, TRIM( DESCRI ) )
    DBSkip()
 ENDDO
// BROWSE( 0, 0, 24, 79 )

// @ 01,01 say STR( len( aArray ) ) COLOR "00/15" 
// inkey(0)
