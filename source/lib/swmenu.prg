...
********************************
function MenuDisplay( MenuList, cCor )

   local nCt, lMouseStatus

   lMouseStatus:= MouseStatus()
   DesligaMouse()

   if (ValType(MenuList) = "A")
      cCor:= iif(ValType(cCor) == ValType(acolor()[1]) .AND. cCor != ;
         Nil, cCor, acolor()[1])
      DispBegin()
      for nCt:= 1 to Len(MenuList)
         /* Testa se e p/ deixar ativo */
         IF MenuList[nCt][14] .OR. SWSet( _SYS_CONFIGSENHAS )
            @ MenuList[nCt][1], MenuList[nCt][2] say MenuList[nCt][3] color cCor
            @ MenuList[nCt][1], MenuList[nCt][2] + MenuList[nCt][5] - 1 say MenuList[nCt][4] color MenuList[nCt][6]
         ELSE
            @ MenuList[nCt][1], MenuList[nCt][2] say MenuList[nCt][3] color "08" + SubStr( cCor, 3 )
            @ MenuList[nCt][1], MenuList[nCt][2] + MenuList[nCt][5] - 1 say MenuList[nCt][4] color "08" + SubStr( MenuList[nCt][6], 3 )
         ENDIF
      next
      DispEnd()
   endif
...
FUNCTION MenuModal( MenuList, nOpcao, Arg3, Arg4 )
   ...
   Local4:= {|| ( DevPos( 24-1, Int( 79 - Len( MenuList[nOpcao][7] ) ) / 2 ), DevOut( MenuList[nOpcao][7], MenuList[nOpcao][10] ) ) }
   ...
   if (!Empty(MenuList))
      ...
      MenuDisplay( MenuList, Local9 )

      do while (.T.)

         Local5[nOpcao]:= iif( Empty( MenuList[nOpcao][7] ), ;
            ScreenSave( 24-1, 0, 24-1, 79 ), ;
            ScreenSave( MenuList[nOpcao][8], MenuList[nOpcao][9], MenuList[nOpcao][8], 79 ) )
         ...
         do while (.T.)
            ...
            IF !Empty( TempoProtecao() )
               /* Ativa a protecao de tela, caso ficar muito tempo parado */
               If ( Seconds() - nProtecao ) / 60 >= TempoProtecao()
                  ProtecaoTela()
                  nProtecao:= Seconds()
               EndIf
            ENDIF

            // cpufree = Melhorar a questao de uso da cpu
            OL_Yield()
               ...
               For nCT:=1 to Len(MenuList)
                   If MouseRow()=MenuList[nCT][1] .AND.;
                      ...
                      IF MouseBOff(2)==2
                         ...
                         VPBox( nMRow - 1, nMCol - 1, nMRow + 4, nMCol + 26, , cCorBest )
                         ...
                         IF Len( Alltrim( PAD( SubStr( MenuList[ nCt ][ 7 ], 20, 20 ), 20 ) ) ) > 0
                           ...
                           IF Len( Alltrim( PAD( SubStr( MenuList[ nCt ][ 7 ], 41, 20 ), 20 ) ) ) > 0
                              ...
                              IF Len( Alltrim( PAD( SubStr( MenuList[ nCt ][ 7 ], 61, 20 ), 20 ) ) ) > 0
                                 @ nMRow+3, nMCol + 5 Say PAD( SubStr( MenuList[ nCt ][ 7 ], 61, 20 ), 20 )
                              ENDIF
                           ENDIF
                         ENDIF
            ...
            IF lMouse .AND. MouseBOff(2)==2
               ...
               DO CASE
                  CASE MouseRow()==2
                     ...
                     @ nMRow+4, nXCol + 5 Say " mento de sistemas desde 1992, trazendo em sua "
                  ...
                  CASE MouseRow() == 0
                     nXCol:= 12
                     nXRow:= 3
                     VPBox( nXRow - 1, nXCol - 1, nXRow + 19, nXCol + 65, , cCorBest )
                     @ nXRow, nXCol Say " INFORMACOES SOBRE O SISTEMA                       "
                     @ nXRow+02, nXCol + 2 Say "                                               "
                     @ nXRow+03, nXCol + 2 Say " Dados do Usuario                              "
                     @ nXRow+04, nXCol + 2 Say _EMP
                     @ nXRow+05, nXCol + 2 Say "                                               "
                     @ nXRow+06, nXCol + 2 Say " Dados do Sistema                              "
                     @ nXRow+07, nXCol + 2 Say " Versao                    = " + _VER
                     @ nXRow+08, nXCol + 2 Say " Diretorio de dados        = " + GDir
                     @ nXRow+09, nXCol + 2 Say " Diretorio de Relatorios   = " + SWSet( _SYS_DIRREPORT )
                     @ nXRow+10, nXCol + 2 Say " "
                     @ nXRow+11, nXCol + 2 Say " Servicos de Suporte & Comunicacao "
                     @ nXRow+12, nXCol + 2 Say " E-Mail de Desenvolvimento = tecnologia@fortunaonline.com.br"
                     @ nXRow+13, nXCol + 2 Say " E-Mail de Suporte         = suporte@fortunaonline.com.br   "
                     @ nXRow+14, nXCol + 2 Say " E-Mail de Comercio        = comercial@fortunaonline.com.br "
                     @ nXRow+15, nXCol + 2 Say " Home Page Oficial         = www.fortunaonline.com.br       "
                     @ nXRow+16, nXCol + 2 Say " Fone(s)                   = 0xx51-3346.7913, 51-9112.8364 e"
                     @ nXRow+17, nXCol + 2 Say "                             51-91576248                    "
         ...
         Endif
            ...
            Case nTecla == K_SPACE
               IF SWSet( _SYS_CONFIGSENHAS )
                  ...
                  //AddSenhas( MenuList )
               ENDIF
            Case nTecla == K_F1 .AND. SWSet( _SYS_CONFIGSENHAS )
               //MenuList[ nOpcao ][ 14 ]:= IF( MenuList[ nOpcao ][ 14 ], .f., .t. )
               AddSenhas( MenuList )
               AddPoderes( .T. )
               GravaSenhas()
...
/* Exibe relogio no canto superior */
...
ELSE
   aFile:= GuardaPoderes()
ENDIF
IF nOpcao == Len( MenuList )  /* Quando for 0>Retorna (ULTIMA OPCAO DO MENU) */
   lExiste:= .T.
ELSE
   FOR nCt:= 1 TO Len( aFile )
       cOpcao:= SetOpcao()
...
@ PROW(), PCOL() Say ZipChr( "--FORTUNA Automacao Comercial - Desde 1992 - Todos Os Direitos Reservados (C)  --" ) + Chr( 13 ) + Chr( 10 )
...
Function Acesso( nTecla )
  Local lReturn:= .F.
  Local cCor:= SetColor(), nCursor:= SetCursor(),;
        cTela:= ScreenSave( 0, 0, 24, 79 ), nArea:= Select()
  Local cModulo
   ...
   ELSEIF !SWSet( _SYS_CONFIGSENHAS )
      IF DBSeek( cModulo + cUserGrupo + Str( nTecla, 4, 0 ) )
         IF SIMNAO=="S"
            lReturn:= .F.
         ELSE
            Aviso( "Desculpe " + Alltrim( SubStr( M->wUsuario, 1, AT( " ", M->wUsuario ) ) )+ ", mas esta tecla nao esta acessivel." )
            Inkey(1)
            lReturn:= .T.
         ENDIF
      ELSE
