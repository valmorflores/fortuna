...
*/
        ...
        IF EpsonVende( cCodFab, cDescri, cUnidade, nQuantidade, nPreco )
           @ PROW()+1,00 Say Chr( 27 ) + Chr( 18 ) + "# sem valor fiscal   " + Chr( 27 ) + Chr( 15 ) + "#"
        ENDIF
        Set( 24, "LPT2" )
        Set( 24, "LPT1" )
...
*/
        ...
        @ PROW()+1,00 Say "     C U P O M   N A O   F I S C A L       "
...
DO CASE
   ...
   CASE Gaveta=="BEMATECH-NAOFISCAL"
        SET( 24, PortaImpressao )
        SET( 20, "PRINT" )
        cComando:= cAbreGaveta
        @ PROW(),PCOL() Say cComando
        SET( 20, "SCREEN" )
...
DO CASE
   ...
   CASE Impressora == "EPSON"
        IF EpsonCanCupom()
           ...
           Set( _SET_DEVICE, "PRINTER" )
           @ PROW()+1,00 Say "----------------------------------------"
           @ PROW()+1,00 Say "CANCELAMENTO CUPOM NAO FISCAL "
           @ PROW()+1,00 Say "-------------------------------"+ TIME()
...
Atualizacao  15/07/98                                                         
-------------------------------------*/
Function impFechaSem( cAcrDes, nPerAcrDes, nVlrAcrDes, nVlrPagar, nValorPago, cMensagem, cCgcCpf, nLocal )
...
DO CASE
   ...
   CASE Impressora=="SIGTRON-FS345"
        ...
        // Percentual esta Negativo arruma
        IF nPerAcrDes < 0
        ...
        Com_Send( impPorta(), cComando )
**        IF Driver="W2000"
           IF AT( ":E", ( cRetorno:= cRetorno + " <241> " + cComando + Com_Read( "", 200 ) ) ) > 0
               /* ERRO */
           ENDIF
**        ENDIF
        ...
        ELSE
           ...
           // Formas de Pagamento
           FOR i:= 1 TO Len( FormasDePagamento )
               IF Left( FormasDePagamento[ i ], 3 )==StrZero( nLocal, 3, 0 )
                  cDesForma:= Upper( Alltrim( SubStr( FormasDePagamento[ i ], 5, Len( FormasDePagamento[ i ] ) ) ) )
                  IF cDesForma=="DINHEIRO"
                     cComando:= Chr( 27 ) + Chr( 242 ) + ForPagDinheiro + "0000000000000" + ForPagDinheiro + " = A Vista" +  Chr( 255 )
                  ELSEIF cDesForma=="CHEQUE"
                     cComando:= Chr( 27 ) + Chr( 242 ) + ForPagCheque   + "0000000000000" + ForPagCheque   + " = Cheque " +  Chr( 255 )
                  ELSEIF cDesForma=="CARTAO"
                     cComando:= Chr( 27 ) + Chr( 242 ) + ForPagCartao   + "0000000000000" + ForPagCartao   + " = Cartao " +  Chr( 255 )
                  ELSEIF cDesForma=="CREDIARIO"
                     cComando:= Chr( 27 ) + Chr( 242 ) + ForPagAPrazo   + "0000000000000" + ForPagAPrazo   + " = A Prazo" +  Chr( 255 )
                  ELSE
                     cComando:= Chr( 27 ) + Chr( 242 ) + ForPagDinheiro + "0000000000000" + ForPagOutros   + " = A Prazo" +  Chr( 255 )
                  ENDIF
                  EXIT
               ENDIF
           NEXT
        ...
        Com_Send( impPorta(), cComando )
**        IF Driver="W2000"
           IF AT( ":E", ( cRetorno:= cRetorno + " <242> " + Com_Read( "", 200 ) ) ) > 0
               /* ERRO */
           ENDIF
**        ENDIF
        ...
        cComando:= cComando + " Evoluthiva - www.lucrativo.com.br            " + Chr( 255 )

        /* Terceiro Comando 243 - Fim do Fechamento */
        Com_Send( impPorta(), cComando )
**        IF Driver="W2000"
           IF AT( ":E", ( cRetorno:= cRetorno + " <243> " + Com_Read( "", 200 ) ) ) > 0
               /* ERRO */
           ENDIF
**        ENDIF
...
*/
        Set( _SET_DEVICE, "PRINTER" )
        nFim:= 0
        @ Prow()+1,00 Say "------- FECHAMENTO CUPOM NAO FISCAL ------"
...
// gelson 30/10        @ PRow()+1,00 Say "SEM VALOR FISCAL ************************ "
