...
DO CASE
   ...
   CASE Impressora=="SIGTRON-FS345"
        ...
        IF AT( "F O R M A   D E   P A G A M E N T O", cMensagem ) > 0
           cComando:= Chr( 27 ) + Chr( 242 ) + "B" + "0000000000000" + " Condicao Especial ---------------------------" + Chr( 255 )
        ...
        cComando:= Chr( 27 ) + Chr( 243 ) + cCgcCpf +  Chr( 13 ) + Chr( 10 ) + cMensagem + Chr( 13 ) + Chr( 10 ) 
   ...
   CASE Impressora=="SIGTRON"
       ...
       IF cCgcCpf <> Nil
          IF Len( ALLTRIM( cCgcCpf ) ) < 14
             cCgcCpf:= "CPF: " + Tran( Space( 3 ) + Alltrim( cCgcCpf ), "@R 999.999.999-99" )
          ELSE
             cCgcCpf:= "CGC: " + Tran( Alltrim( cCgcCpf ), "@R 99.999.999/9999-99" )
          ENDIF                                      
       ELSE
       ...
       cMensagem:= cCgcCpf + Chr( 13 ) + Chr( 10 ) + cMensagem + Chr( 13 ) + Chr( 10 ) + " SOFT&WARE           www.fortunaonline.com.br"
...
// ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
   ...
   Function fImpMoeda (nPgtDin, nPgtTic, nPgtChq, nPgtCar, nPgtPra, nPgtOut)  //AQUI 04 = 32 72 34
...
DO CASE
   ...
   CASE Impressora=="SIGTRON-FS345"
        ...
        cComando:= Chr( 27 ) + Chr( 219 ) + ForPagAPrazo + ForPagAPrazo + ImpCupomAtual( "RETORNAR" ) + StrZero( nValor * 100, 12, 0 )
        Com_Send( impPorta(), cComando )

        /* Mensagens a serem impressas no corpo do cupom NAOFISCAL vinculado */
        FOR nCt:= 1 TO 2
        ...
        FOR nCt:= 1 TO Len( cCrediario )
            IF SubStr( cCrediario, nCt, 1 )==Chr( 255 ) .OR. SubStr( cCrediario, nCt, 1 )==Chr( 10 )
               cMsg:= cMsg + ""
...
DO CASE
   ...
   CASE Impressora=="Nenhuma" .OR. Impressora=="EPSON"
        nArea:= Select()
        DBSelectAr( 133 )
        Use &DiretorioDeDados\CUPOM___.VPB Alias XXC Shared
