...
Function Sistema( cVersao )
 ...
 //  MReset()
  ...
  // MCurOn()
   ...
   ELSE
      PicRead( 000, 000, 129, "FUNDO16.BMP" )
      PicRead( 782, 055, 001, "LOGO16.BMP" )
   ENDIF
   ...
   WHILE !LastKey()==K_ESC .AND. lFim == .F.
      ...
      SayString( 830, 580, 4, 0, 0, cVersao + " " + os() ) // Versao do Programa
...
Function Browse( lFim, lCupomPendente, lVersaoConflito, cVersao, nHandInfo, nHandProd, nHandDisp, nHandLogo )
 ...
 // ENDIF
  ...
  oTab:addcolumn(tbcolumnnew(,{|| PAD( " " + Left( MPR->DESCRI, 35 ) + " " + MPR->ORIGEM, 45 ) }))
  oTab:AUTOLITE:=.f.
  oTab:dehilite()
...
//       cBusca:= ""; lAchou := .T.
      ...
      IF !nUltReg==RECNO() .OR. lSubTotal
         ...
         IF ( !ALLTRIM( MPR->Detal1 ) == "" ) .OR. ( !ALLTRIM( MPR->Detal2 ) == "" ) .OR.;
            ...
            loadcSet( 0, "AGSYSD10.PTX" )
            SayString( 37, 957, 4, 0, 00, MPR->DESCRI )
            SayString( 34, 942, 4, 0, 11, MPR->DETAL1 )
            SayString( 34, 925, 4, 0, 11, MPR->DETAL2 )
            SayString( 34, 908, 4, 0, 11, MPR->DETAL3 )
         ELSE
         ...
         SayString( 400, 660, 4, 0, IF( EMPTY( MPR->SALDO_ ), 15, 1 ), IIF (EMPTY (MPR->SALDO_) = .F., Alltrim( Str( MPR->SALDO_, 10, 3 ) ), "~~"))
         SayString( 600, 660, 4, 0, IF( EMPTY( MPR->SALDO_ - MPR->RESERV ), 15, 1 ), IIF (EMPTY (MPR->SALDO_ - MPR->RESERV) = .F., Alltrim( Str( MPR->SALDO_ - MPR->RESERV, 10, 3 ) ), "~~"))
      ...
      // ESPERA TECLA E VERIFICA ARQUIVOS
      // GERADOS POR SERIAL POR INTERMEDIO DO
      // UTILITARIO COM2SAY
      WHILE ( nTecla:= Inkey() )==0 // #01
          IF FILE( "COM2SAY.BAT" ) .AND. FILE( "COM2SAY.EXE" )
             RUN com2say.bat
             IF FILE( "KEY.TXT" )
                cTecla:= SubStr( Memoread( "KEY.TMP" ), 1, 1 )
                Keyboard &cTecla
                nTecla:= inkey(0)
                FErase( "KEY.TMP" )
                EXIT
             ELSEIF File( "COM2SAY.TXT" )
                cTecla:= Memoread( "COM2SAY.TMP" )
                Keyboard &cTecla
                nTecla:= inkey(0)
                FErase( "COM2SAY.TMP" )
                EXIT
      ...
      DO CASE
         ...
         CASE nTecla == K_ENTER // #01
               ...
               IF nStatus==1 .OR. nStatus==2 .OR. nStatus==8 .OR. nStatus==9
                  ...
                  IF nStatus == 1 .OR. VendaFile( _ULTIMOREGISTRO ) <= 0
                     ...
                     DispCupom( " ", 0)
                     DispCupom( " ", 0)
                     DispCupom( "VENDA INICIADA AS " + HORA() + "Hs.", 0 )
                     DispCupom( "Cupom Fiscal No. " + NumeroCupom( ), 0 )
                     DispCupom( "Produto                                               Preco", 5)
                     DispCupom( "======================================= ==========", 0)
                  ENDIF
         ...
         CASE nTecla==K_F8
              ...
              BoxFill( 415, 387+145, 150, 40, 20 + 32, 0 )
                            BoxFill( 920, 387+145, 100, 40, 20 + 32, 0 )  // gelson
              BoxFill( 415, 327+145, 315, 40, 20 + 32, 0 )
                            BoxFill( 810, 327+145, 315, 40, 20 + 32, 0 )
//                            BoxFill( 800, 327+145, 315, 40, 20 + 32, 0 )
              ...
              @ 15, 45 Say "Vendedor"
              @ 15, 55 Get nVendedor Pict "9999"                  // gelson
...
// FIM DAS FLORZINHAS!
              ...
              IF !LastKey() == K_ESC
                 ...
                 IF Impressora == "EPSON"
                    SET( 24, PortaImpressao )
                    SET( 20, "PRINTER" )
                    @ PROW()+1,00 SAY CHR( 255 )
                    SET( 20, "SCREEN" )
                    INKEY( 0.50 )
                 ENDIF
                       ...
                       FOR nCt:= 1 TO Len( aParcelas )
                           ...
                           IF !( ASCAN( NaoImprimeCarne, aParcelas[ nCt ][ 8 ] ) > 0 )
                              ...
                              IF aParcelas[ nCt ][ 5 ] <> CTOD( "  /  /  " )
                                 cCrediario:= cCrediario + "*[AUT]* Pagamento efetuado em " + DTOC( aParcelas[ nCt ][ 5 ] ) + CRLF
                              ENDIF
                              // ESPACO
                              FOR nLin:= 1 TO 5
                                  cCrediario:= cCrediario + " " + CRLF
                     ...
                     ELSE
                        ...
                        IF Impressora=="SIGTRON-FS345" .AND. !CarneInterno
                           ...
                           IF nPerAcrDes <> 0 .OR. nVlrAcrDes <> 0
                              cCrediario:= ""
                           ENDIF
                           cRetorno:= ImpFechaSem( cAcrDes, nPerAcrDes, nVlrAcrDes, nTotal, nValorPago, cMensagem + cCrediario, ALLTRIM( cCliCGCCPF ), nBanco )

                        ELSEIF Impressora=="SWEDA"
                           // Inibida Informacoes de Crediario - Somente mensagens
                           ImpFechaSem( cAcrDes, nPerAcrDes, nVlrAcrDes, nTotal, nValorPago, cMensagem, ALLTRIM( cCliCGCCPF ), nBanco )

                        ELSEIF Impressora=="SCHALTER" .OR. Impressora=="SCHALTER-II"
                           // Fecha Cupom com Crediario Interno
                           ImpFechaSem( cAcrDes, nPerAcrDes, nVlrAcrDes, nTotal, nValorPago, cMensagem + Propaganda, ALLTRIM( cCliCGCCPF ), nBanco )
                        ...
                        ELSE
                           ...
                           cRetorno:= ImpFechaSem( cAcrDes, nPerAcrDes, nVlrAcrDes, nTotal, nValorPago, cMensagem, ALLTRIM( cCliCGCCPF ), nBanco )
                 ...
                 IF (!LastKey() == K_F10 .AND. !LastKey() == K_CTRL_F10)
                    ...
                    IF Len( ALLTRIM( cCrediario ) ) > 100 .AND. lPrinter .AND. ;
                       ...
                       IF (!Impressora == "BEMATECH-MP20FI-II")
                            ...
                            lOk:= .F.
                            WHILE !lOk
                                lOk:= ImpCarne( cCrediario, StrZero( nCliCodigo, 6, 0 ) + "-" + cCliDescri,;
                                         IF( !Empty( cCliCgcCpf ), ;
                                         Tran( cCliCgcCPF,;
                                         "@R 99.999.999/9999-99" ),;
                                          Tran( cCliCgcCpf, "@R 999.999.999-99" ) ), nValorPagar )
                                // Se deu falha na impressao do carne adicional avisa o cliente
                                IF !lOk
                                   SWGAviso( "FALHA NA IMPRESSAO DO CARNE",;
                                        {{0, " "},;
                                         {0, "   A Comunicacao com a impressora falhou!"},;
                                         {0, " "},;
                                         {0, "   Para continuar tentando pressione [ENTER]         "},;
                                         {0, " "},;
                                         {0, "   Para abandonar a impressao do carne pressione [ESC]"},;
                                         {0, " "}}, 4, 3, 300, 200, 800, 1220)
                                   IF LastKey()==K_ENTER
                                      lOk:= .F.
                                   ELSE
                                      lOk:= .T.
...
// ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
   ...
   Function fVerLimCred( lLimCredAvis, nCliCodigo, nTotal, nSalLimCre )
      ...
      Local cTamAnt:= SetColor( ), nCursor:= SetCursor( ), nArea:= Select(), ;
            nOrdem:= IndexOrd( ), nAtraso:= 0
      Local lRet := .T.
      Local aTitulos:= {}, aAviso:= {}
      ...
      IF File( DiretoriodeDados + "\DPAIND05.NTX" )
         ...
         DPA->( DBSetOrder( 5 ) )
         IF DPA->( DBSeek( CLI->CODIGO ) ) .AND. CLI->CODIGO > 0
            nAtraso:= 0
            WHILE DPA->CLIENT == CLI->CODIGO .AND. !DPA->( EOF() )
               IF TRIM( DPA->NFNULA ) = ""
                  IF DPA->( VENC__ ) < DATE() .AND. EMPTY( DTQT__ )
                     nAtraso:= nAtraso + DPA->VLR___
                     // DEBUG: Aviso
                    AAdd( aTitulos, {1, " Venc: " + dtoc( DPA->VENC__ ) + " Vlr:" + ;
		              tran( DPA->VLR___, "@E 999,999.99" ) + " Dias: " + ;
			                   StrZero( DATE() - DPA->VENC__, 06, 0 ) } )
                  ENDIF
               ENDIF
               DPA->( DBSkip() )
            ENDDO
            IF nAtraso > 0
   	        aAviso:= {{0, " Cliente possui pendencias no total de " + ALLTRIM( TRAN(nAtraso, "@E 99,999,999.99" ))}}
                 FOR i:= 1 TO Len( aTitulos )
		     AADD( aAviso, aTitulos[i] )
		 NEXT
	         AADD( aAviso, {0, " "} )
                 AADD( aAviso, {0, " "} )				
		 // Exibe o aviso
		 SWGAviso( "DEBITO PENDENTE - ATRASO", aAviso, 7, 4, 300, 200, 800, 1220)		 
                 // Grava Log do aviso
                 SysLog( "AVISO!" + Strzero( CLI->CODIGO, 8, 0 ) + " - " + CLI->DESCRI + ;
                     " Pendencia de " + ALLTRIM( TRAN( nAtraso, "@E 99,999,999.99" )) + "." )
            ENDIF
         ENDIF
      ENDIF
      // FIM DA VERIFICACAO DE PENDENCIAS
...
ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
*/
Function BuscaCli( nCliCodigo, cCliDescri, cCliEndere,  cCliBairro,  cCliCidade,  cCliEstado, cCliFone, cCliCodCep,  cCliCGCCPF, cCliIEstadual, nCliVendedor, cCliObser1, cCliObser2, cCliObser3, cCliObser4, cCliAgenda, GetList, cVendedor )       // gelson (cVendedor)
Local cFontBrowse:= "VSYS14.PTX"
Local nCt:= 0
...
/*
        ...
        Locate For CODIGO=nCliVendedor
        cVendedor:=DESCRI
        DBSelectAr( _COD_CLIENTE )
*/
        FOR nCt:= 1 TO Len( GetList )
  ...
  ELSE
     ...
     IF EMPTY( cCliDescri )
        ...
        oTb:addcolumn(tbcolumnnew(,{|| PAD( " " + Str( If( CLI->VENIN1 <= 0, CLI->VENEX1, CLI->VENIN1 ), 10 ), 65 ) }))    // gelson
        oTb:AUTOLITE:=.f.
        oTb:dehilite()
        WHILE .T.
            loadcSet( 0, cFontBrowse )
            ...
            DO CASE
               CASE CHR( nTecla )=="+"
                    ...
                    CUP->( DBSetOrder( 4 ) )
                    IF CUP->( DBSeek( CLI->CODIGO, .T.  ) )
                       nLin:= 10
                       // Enquanto Cliente Localizado
                       WHILE CUP->CLIENT==CLI->CODIGO 
               ...
               CASE nTecla==K_ENTER
                    ...
                    ELSE
                       nCliVendedor:= VENIN1
                    ENDIF
                    IF ( fVerLimCred( .T., @nCliCodigo, 1, CLI->LIMCR_ ) == .T. )
                        EXIT
                    ENDIF
               OTHERWISE                ;tone(125); tone(300)
            ENDCASE
   ...
   Function fAuxilio (cVersao)
      ...
      SWGAviso( "AUXILIO                                                              4 de 4",;
              {{0, "   Confirmacao de Preco . . . . . . . . . . . . . . . . . . . . . . . . . . [F12]"},;
               {0, "   Auxilio  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [HOME]"},;
               {0, "   Impressao de Informacoes da venda (PDV.INI)  . . . [CTRL+TAB]"},;
               {0, " "},;
...
Function Carne()
 Local CRLF:= CHR( 13 ) + CHR( 10 )
   ...
   ENDIF
              ...
              /// Carrega carne apartir do cupom e informacoes
              /// financeiras ja gravadas referentes ao ultimo cupom

              // Atualiza Informacao do Cupom Atual
              ImpCupomAtual( "ATUALIZAR" )

              cCupom:= ImpCupomAtual( "RETORNAR" )

              SWGAviso( "INFORMACAO RETORNADA DA IMPRESSORA",;
                                        {{ 15, "CUPOM => " + cCupom },;
                                         { 15, "   " },;
                                         {0, "Modulo de Impressao de Carne Adicional por Cupom "   },;
                                         {0, "   " },;
                                         { 15, "   Para Imprimir o carne pressione [ENTER]         "},;
                                         { 15, "   Para abandonar a impressao do carne pressione [ESC]"},;
                                         {0, " "}}, 4, 3, 300, 200, 800, 1220)
              ...
              WHILE !( lOk )
                         ...
                         CUP->( DBSetOrder( 1 ) )
                         CUP->( dbSeek( Val( cCupom ) ) )

                         DPA->( DBSetOrder( 1 ) )
                         DPA->( DBSeek( Val( cCupom ) ) )

                         //// LancaReceber( aParcelas, nCliCodigo, cCliDescri )

                         cCrediario:= ""
                         nCliCodigo:= DPA->CLIENT
                         cCliDescri:= DPA->CDESCR
                         cCliCGCCPF:= ""
                         nValorPagar:= cup->VLRTOT
                         nParcela:= 0
                         aParcelas:= {}
                         WHILE DPA->CODNF_ == Val( cCupom )
                             AADD( aParcelas, {,,,,,,,,,,,,} )
                             nCt:= LEN( aParcelas )
                             aParcelas[ nCt ][ 1 ]:= DPA->PERC__
                             aParcelas[ nCt ][ 2 ]:= DPA->PRAZ__
                             aParcelas[ nCt ][ 3 ]:= DPA->VLR___
                             aParcelas[ nCt ][ 4 ]:= DPA->VENC__
                             aParcelas[ nCt ][ 5 ]:= DPA->DTQT__
                             aParcelas[ nCt ][ 6 ]:= DPA->BANC__
                             aParcelas[ nCt ][ 7 ]:= ""
                             aParcelas[ nCt ][ 8 ]:= DPA->LOCAL_
                             DPA->( DBSkip() )
                         ENDDO

                 /// Exibe as parcelas na tela
                 aAviso:= {}
                 aadd( aAviso, { 4, "CUPOM FISCAL: " + impCupomAtual( "RETORNAR" ) } )
                 FOR nCt:= 1 TO Len( aParcelas )
                     nPercentual:= aParcelas[ nCt ][ 1 ]
                     nPrazo:=      aParcelas[ nCt ][ 2 ]
                     nParcela:=    aParcelas[ nCt ][ 3 ]
                 ...
                 IF Len( aParcelas ) > 0
                    SWGAviso( "CONDICOES DE PAGAMENTO                                     [F10]Anula", aAviso, 7, 3 )
                 ENDIF
                         IF !CarneInterno .AND. Len( aParcelas ) > 0
                         ...
                         IF !lOk
                                   SWGAviso( "FALHA NA IMPRESSAO DO CARNE",;
                                        {{7, " "},;
                                         {7, "   A Comunicacao com a impressora falhou!"},;
                                         {7, " "},;
                                         {7, "   Para continuar tentando pressione [ENTER]         "},;
                                         {7, " "},;
                                         {7, "   Para abandonar a impressao do carne pressione [ESC]"},;
                                         {7, " "}}, 4, 3, 300, 200, 800, 1220)
                                   IF LastKey()==K_ENTER
                                      lOk:= .F.
                                   ELSE
                                      lOk:= .T.
   ...
   */
   ...
   Function SysLog( Mensagem )
      Run( "@ECHO &Msg >>syslog.txt" )
