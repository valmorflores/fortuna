...
lPrinter:= .T.
IF Impressora $ "Nenhuma-Treinamento"
   @ Row(), 00 Say "Nenhuma Impressora conectada..."
   lPrinter:= .F.
   Inkey(.5)
   Cls
ELSE
   IF Status() == 1 .AND. !( Impressora $ "Nenhuma-Treinamento" )
      Quit
   ELSE
      Cls
   ENDIF
...
IF !File( DiretorioDeDados-"\MPRPDV01.NTX" )
   Index On SUBSTR( INDICE, 1, 3 ) + STRZERO( CODFOR, 3, 0 ) + SUBSTR( INDICE, 4, 4 ) + Space( 3 ) To &DiretorioDeDados\MPRPDV01.NTX
ENDIF
Set Index To &DiretorioDeDados\MPRIND02.NTX,;
             &DiretorioDeDados\MPRIND04.NTX,;
             &DiretorioDeDados\MPRIND01.NTX,;
             &DiretorioDeDados\MPRPDV01.NTX
              ...
              IF !lCancelaItem
                 ...
                 IF lPrinter
                    lRetorno:= impVendeProduto( cCodFab,;
                                 ...
                                 cUnidade )
                 ENDIF
                 IF lRetorno .OR. ( Impressora $ "Nenhuma-Treinamento" ) .OR. Len( aProduto ) <= 1
...
Local nValorIcm:= 0, nValorIpi:= 0, nValorBase:= 0
  Sele B
  DBCloseArea()
  Use &DiretoriodeDados\CUPOM___.DBF Alias CUP Shared
  ...
  Sele C
  Use &DiretoriodeDados\CUPOMAUX.DBF Alias CPA Shared
...
Local nCt
  Sele G
  Use &DiretoriodeDados\CESTOQUE.DBF Alias EST Shared
...
Local nCt
  Sele C
  Use &DiretoriodeDados\CAIXA___.DBF Alias CX Shared
...
Local nCt
  Sele B
  Use &DiretoriodeDados\CESTOQUE.DBF Alias ES1 Shared
  Set Index To &DiretoriodeDados\ESTIND01.NTX,;
               &DiretoriodeDados\ESTIND02.NTX,;
               &DiretoriodeDados\ESTIND03.NTX,;
               &DiretoriodeDados\ESTIND04.NTX
...
Local nArea:= Select()
   DBSelectAr( 33 )
   Use &DiretoriodeDados\CLIENTES.DBF Alias CLI Shared
...
Local nArea:= Select()
   DBSelectAr( 34 )
   IF !Used()
      Use &DiretoriodeDados\VENDEDOR.DBF Alias VEN Shared
